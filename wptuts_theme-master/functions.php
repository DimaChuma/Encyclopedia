...
function wptuts_the_breadcrumb(){
	global $post;
	if(!is_home()){ 
	   echo '<li><a href="'.site_url().'"><i class="fa fa-home" aria-hidden="true"></i>'. esc_html__('Головна', 'wptuts'). '</a></li> <li> / </li>';
		if(is_single()){ // posts
		the_category(', ');
		echo " <li> / </li> ";
		echo '<li>';
		...
		elseif (is_search()) { // search pages
			echo esc_html__('Результат пошуку "', 'wptuts') . get_search_query() . '"';
		}
		elseif (is_tag()) { // tags
	   ...
	   if($pageNum>1)
	      echo '<li><a href="'.site_url().'"><i class="fa fa-home" aria-hidden="true"></i>'. esc_html__('Головна', 'wptuts'). '</a></li> <li> / </li> <li>'.$pageNum.'- page</li>'; //fix close <i> and add <li>-page</li>
	   else
	      echo '<li><i class="fa fa-home" aria-hidden="true"></i>'. esc_html__('Головна', 'wptuts'). '</li>';
	}
}
/**
* Pagination
